...
function setupLabjackBuffers(Vdac0, Vdac1, BitFIO3,BitFIO4)
    ...
    dacNumber = 0
    dBytesVolt = Vdac0*caliInfo.ccConstants[17 + dacNumber*2] +
            caliInfo.ccConstants[18 + dacNumber*2];
    if dBytesVolt > 65535
        dBytesVolt = 65535
    elseif dBytesVolt < 0
        dBytesVolt = 0
    ...
    dacNumber = 1
    dBytesVolt = Vdac1*caliInfo.ccConstants[17 + dacNumber*2] +
            caliInfo.ccConstants[18 + dacNumber*2];
    if dBytesVolt > 65535
        dBytesVolt = 65535
    elseif dBytesVolt < 0
...
function labjackReadWrite(Vdac0, Enable0)
    if (powerSupply == :Ultravolt)
        ...
        (Enable0 == true) || (Vdac0 = 0.0)
        setLJTDAC(HANDLE,caliInfoTdac,2,Vdac0,0.0)
    end
