...
function oneHz_smps_loop()
    ...
    N = (useCounts == true) ? deepcopy(tenHz_df[:N1cpcCount]) : deepcopy(tenHz_df[:N1cpcSerial])
    ...
    correct = @. x -> -lambertw(-x*flowRate1*16.666τ*1e-6,0)/(flowRate1*16.6666*τ*1e-6)
    currentDiameter = get_gtk_property(gui["currentD"], :text, String) |> x->parse(Float64,x)
	if length(N[state .== :SCAN]) > τᶜ*10 + 1
		...
		global ℝ =  resample((mDp,mN), (δˢᵐᵖˢ.Dp, δˢᵐᵖˢ.De))
        ...
        maxD = get_gtk_property(gui["StartDiameter"], :text, String) |> x->parse(Float64,x)
        minD = get_gtk_property(gui["EndDiameter"], :text, String) |> x->parse(Float64,x)
...
function tenHz_smps_loop()
    ...
    RH1,T1,Td1 = (channel == -1) ? (missing,missing,missing) : AIN2HC(AIN,channel+1,channel+2)
    ...
    RH2,T2,Td2 = (channel == -1) ? (missing,missing,missing) : AIN2HC(AIN,channel+1,channel+2)
	...
	push!(tenHz_df, Dict(:Timestamp => ts, 
...
function oneHz_generic_loop()
    ...
    (typeof(Nserial1) == Missing) || addpoint!(t,Nserial1,plotParticle,gplotParticle,1,true)
	(typeof(Nserial2) == Missing) || addpoint!(t,Nserial2,plotParticle,gplotParticle,2,true)
...
function oneHz_data_file()
    ...
    push!(oneHz_df, Dict(:Timestamp => ts, 
                         :Unixtime => datetime2unix(ts),
                         :Int64time => Dates.value(ts),
